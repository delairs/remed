<template>
  <div>
    <p>ini Personal Chat</p>
    <p>{{ currUser.user.email }}</p>
    <p>{{ currPersonalGroupID }}</p>
    <p>{{ sele }}</p>
    <p>{{ message }}</p>
...
export default {
  setup() {
    ...
    const currPersonalGroupID = computed(
      () => store.getters.computedPersonalGroupID
    );
    ...
    const fetchChatHistory = async () => {
      try {
        const res = await fetch(
          "http://localhost:3000/api/personalchat?where[PersonalID][equals]=" +
            currPersonalGroupID.value,
          {
            method: "GET",
            credentials: "include",
            headers: {
    ...
    const sendMessage = async () => {
      try {
        const res = await fetch("http://localhost:3000/api/personalchat", {
          method: "POST",
          credentials: "include",
          headers: {
            "Content-Type": "application/json",
          ...
          body: JSON.stringify({
            ...
            PersonalID: currPersonalGroupID.value,
