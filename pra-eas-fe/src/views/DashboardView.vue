    ...
    <div>
      ...
      <form @submit.prevent="personalChat">
        <div class="mb-3">
          ...
          <input
            v-model="currPersonalUser"
            class="form-control"
            id="currPersonalUser"
            name="currPersonalUser"
            required
          />
        </div>
    ...
    const getPC = async () => {
      try {
        ...
        if (res.status === 401) {
          ...
          const json = await res.json();
          allPC.value = json.docs.filter((group) =>
            group.UserID.some((user) => user.id === currUser.value.user.id)
          );
          console.log(allPC);
          return allPC;
        }
